...
public class Mypage extends JFrame implements ActionListener,ItemListener {
	...
	public  Mypage(){
		 ...
		 try { Class.forName("com.mysql.cj.jdbc.Driver");
		  ...
		  String strSql="SELECT * FROM user;"; //where id=getText.jTextI() //jTextI에서 친 아이디로 정보가져오기,,,
		...
		handicap.setBounds(150,300,70,20);
		 ...
		 update = new JButton("수정하기");
			...
			ct.add(mov);
			ct.add(del);
			ct.add(smallcar); ct.add(special);
			ct.add(femail); ct.add(handicap);
	...
	strSql="UPDATE user SET name='"+t_name+"',id='"+t_id+"',passwd='"+t_passwd+"',car_num='"+t_carn+"',phone_num='"+t_call+"',card_num='"+t_card+"',email='"+t_email+"'WHERE id='"+id2.getText()+"';" ; //!한줄로 작성!
	dbSt.executeUpdate(strSql); //개인정보 수정
	strSql = "UPDATE user_special_needs SET id='"+t_id+"',woman='"+is_femail+"',small_car='"+is_smallcar+"',handicap='"+is_handicap+"'WHERE id='"+id2.getText()+"';";				
